...
import { MapPin, RefreshCw, Clock, Navigation, Send, Crosshair, User } from 'lucide-react';
import toast from 'react-hot-toast';
import { driverApi } from '../../api/driverApi';
import { trackingApi } from '../../api/trackingApi';
import type { DriverResponse, LocationResponse } from '../../types';
...
const FitBounds: React.FC<{ locations: LocationResponse[] }> = ({ locations }) => {
  const map = useMap();
  useEffect(() => {
...
const TrackingPage: React.FC = () => {
  ...
  const handlePostLocation = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const polylinePositions = locations.map((l) => [l.latitude, l.longitude] as [number, number]);
          ...
          {selectedDriverId && (
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              {locationsLoading ? (
                <div style={{ padding: 16 }}>
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="skeleton" style={{ height: 14, marginBottom: 10, width: `${50 + Math.random() * 40}%` }} />
                  ))}
                </div>
              ) : locationsError ? (
