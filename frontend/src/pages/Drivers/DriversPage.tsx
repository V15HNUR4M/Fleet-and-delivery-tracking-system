...
import type { DriverAvailability, DriverRequest, DriverResponse } from '../../types';
...
const defaultForm: DriverRequest = {
  name: '',
  mobile: '',
  licenseNumber: '',
  licenseExpiry: '',
...
const DriversPage: React.FC = () => {
  ...
  const [availFilter, setAvailFilter] = useState<DriverAvailability | ''>('');
  ...
  const [formErrors, setFormErrors] = useState<Partial<Record<keyof DriverRequest, string>>>({});
  const [submitting, setSubmitting] = useState(false);

  // Availability modal
  const [availModalOpen, setAvailModalOpen] = useState(false);
  const [availTarget, setAvailTarget] = useState<DriverResponse | null>(null);
  const [newAvail, setNewAvail] = useState<DriverAvailability>('AVAILABLE');
  ...
  const validate = (): boolean => {
    const errs: Partial<Record<keyof DriverRequest, string>> = {};
  ...
  const openAvailModal = (d: DriverResponse) => {
    setAvailTarget(d);
    setNewAvail(d.availability);
    setAvailModalOpen(true);
  };

  const handleAvailUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    <div>
      ...
      <div className="toolbar">
        ...
        <select
          ...
          value={availFilter}
          onChange={(e) => { setAvailFilter(e.target.value as DriverAvailability | ''); setPage(0); }}
        ...
        <form onSubmit={handleAvailUpdate}>
          <div className="form-group">
            ...
            <select
              ...
              value={newAvail}
              onChange={(e) => setNewAvail(e.target.value as DriverAvailability)}
            >
