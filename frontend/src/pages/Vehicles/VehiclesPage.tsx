...
import type { VehicleRequest, VehicleResponse, VehicleStatus } from '../../types';
...
const VehiclesPage: React.FC = () => {
  ...
  const [formErrors, setFormErrors] = useState<Partial<VehicleRequest>>({});
  const [submitting, setSubmitting] = useState(false);

  // Status modal
  const [statusModalOpen, setStatusModalOpen] = useState(false);
  const [statusTarget, setStatusTarget] = useState<VehicleResponse | null>(null);
  ...
  const validateForm = (): boolean => {
    const errs: Partial<Record<keyof VehicleRequest, string>> = {};
    if (!form.regNumber.trim()) errs.regNumber = 'Registration number is required.';
    if (!form.type) errs.type = 'Vehicle type is required.';
    if (!form.capacityKg || form.capacityKg <= 0) errs.capacityKg = 'Capacity must be greater than 0.' as any;
    setFormErrors(errs as any);
  ...
  const openStatusModal = (v: VehicleResponse) => {
    setStatusTarget(v);
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input
                ...
                required
              />
              {formErrors.capacityKg && <div className="form-error">{String(formErrors.capacityKg)}</div>}
            </div>
            <div className="form-group">
